                ...
                <div class="lg:col-span-2 space-y-8">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                        ...
                        <ul class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                            <x-car-features :value="$car->features?->abs ?? false">{{ __('features.abs') }}</x-car-features>
                            <x-car-features :value="$car->features?->air_conditioning ?? false">{{ __('features.air_conditioning') }}</x-car-features>
                            <x-car-features :value="$car->features?->power_windows ?? false">{{ __('features.power_windows') }}</x-car-features>
                            <x-car-features :value="$car->features?->power_door_locks ?? false">{{ __('features.power_door_locks') }}</x-car-features>
                            <x-car-features :value="$car->features?->cruise_control ?? false">{{ __('features.cruise_control') }}</x-car-features>
                            <x-car-features :value="$car->features?->bluetooth_connectivity ?? false">{{ __('features.bluetooth_connectivity') }}</x-car-features>
                            <x-car-features :value="$car->features?->gps_navigation ?? false">{{ __('features.gps_navigation') }}</x-car-features>
                            <x-car-features :value="$car->features?->heated_seats ?? false">{{ __('features.heated_seats') }}</x-car-features>
                            <x-car-features :value="$car->features?->climate_control ?? false">{{ __('features.climate_control') }}</x-car-features>
                            <x-car-features :value="$car->features?->rear_parking_sensors ?? false">{{ __('features.rear_parking_sensors') }}</x-car-features>
                            <x-car-features :value="$car->features?->leather_seats ?? false">{{ __('features.leather_seats') }}</x-car-features>
                        </ul>
                    </div>
                </div>
