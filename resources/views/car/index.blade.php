<x-app title="{{ __('My Cars') }}">
    <div class="container mx-auto px-4 py-8">
        <div class="flex justify-between items-center mb-8">
            <h1 class="text-3xl font-black text-gray-900 tracking-tight">{{ __('My Garage') }}</h1>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-50 border-b border-gray-100 text-xs uppercase text-gray-500 font-semibold tracking-wider">
                            <th class="p-6">{{ __('Vehicle') }}</th>
                            <th class="p-6">{{ __('Added') }}</th>
                            <th class="p-6">{{ __('Price') }}</th>
                        ...
                        <tr class="hover:bg-gray-50/50 transition-colors group">
                                ...
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-xs text-gray-500 mt-1">
                                            {{ $car->details_completed ? __('Details Complete') : __('Details Incomplete') }}
                                        </div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="p-6">
                                <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <a href="{{ route('car.edit', $car) }}" class="p-2 text-gray-400 hover:text-primary hover:bg-primary-50 rounded-lg transition-all" title="{{ __('Edit') }}">
                                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                        </svg>
                                    </a>
                                    <form action="{{ route('car.destroy', $car) }}" method="POST" onsubmit="return confirm('{{ __('Are you sure you want to delete this car?') }}');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all" title="{{ __('Delete') }}">
                                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="text-center py-16 px-4">
                ...
                <h3 class="text-lg font-bold text-gray-900 mb-1">{{ __('No cars listed yet') }}</h3>
                <p class="text-gray-500 mb-6 max-w-sm mx-auto">{{ __('It looks like you haven\'t listed any cars for sale yet. Get started today!') }}</p>
